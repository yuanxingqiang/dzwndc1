...
class register_ctl {
	...
	function on_register() {
		...
		if($_G['uid']) {          //系统已经拥有用户uid，说明实际上是登录状态
			$ucsynlogin = $this->setting['allowsynlogin'] ? uc_user_synlogin($_G['uid']) : '';
		...
		} elseif(!$this->setting['regclosed'] && (!$this->setting['regstatus'] || !$this->setting['ucactivation'])) {          //这几个在$_G['setting']中都没有，说明是一种未登录状态
		...
		if($this->setting['regverify']) {          //开启了'全局-注册访问-注册-新用户注册验证'的话
		...
		$invitestatus = false;
		if($this->setting['regstatus'] == 2) {         //不存在的一种状态
