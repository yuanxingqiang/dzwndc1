...
class table_forum_post extends discuz_table
{
	...
	public function find_limit_forum_and_image($page=0){
        $result = DB::fetch_all('select pre_forum_post.*,pre_forum_threadimage.attachment as pic_url from pre_forum_post left join pre_forum_threadimage on pre_forum_post.tid = pre_forum_threadimage.tid'.DB::limit($page==0?$page:$page*8-8,8),array());
        return $result;
    }
	
	public function find_newest_10_forums(){
        $pre_result = DB::fetch_all('select * from %t order by dateline desc', array($this->_table));
		$result;
